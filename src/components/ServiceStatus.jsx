...
import { CCard, CCardTitle, CCardContent, CModal, CCardActions, CIcon, CButton } from '@cscfi/csc-ui-react';
...
const StatusModal = (props) => {
  ...
  return (
    <CModal
      ...
      style={{ overflow: 'scroll', "--_c-modal-width": '50vw !important' }}
      className='!overflow-hidden !w-min !m-0 border-none! !h-[50vh]'
      width='1000px'
      value={isModalOpen}
      dismissable
      ...
      <CCard style={{ overflow: 'scroll' }} className='text-on-white !m-0 lg:mx-[100px] flex flex-col overflow-scroll lg:!overflow-hidden max-h-[80vh] '>
        ...
        <CCardActions>
          <div className='flex flex-col sm:flex-row sm:justify-between w-full gap-4'>
              ...
              <CButton className="self-end w-min"  onClick={() => setIsModalOpen(false)} text>Close</CButton>
          </div>
        </CCardActions>
      </CCard>
    </CModal>
...
export const ServiceStatus = (props) => {
  ...
  const handleCardClick = (qc) => {
    setModalProps({ ...qc, status, isModalOpen: true, setIsModalOpen: (v) => setModalOpen(v) });
    setModalOpen(true);
  };
