...
Encoded version 1 is taken from a research of Pokharel and Lidar [[12]](#references). Encoded Version 2 builds upon Version 1 but employs a different encoding method that better matches the layout of the Helmi, our 5-qubit QPU. Both circuits perform the same function, but Circuit 2 aligns better with the star-shaped topology, eliminating the need for SWAP gates when running on Helmi.
